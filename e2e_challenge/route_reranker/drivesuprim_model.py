...
class DriveSuprimModel(nn.Module):
    ...
    def forward(self,
        ...
        if self.use_multi_stage:
            ...
            _rr = getattr(self._trajectory_offset_head, '_last_route_rerank', None)
            if _rr is not None:
                trajectory['route_rerank'] = _rr
    ...
    def forward(self, img_feat, refinement_dict, route=None, route_mask=None) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.num_stage):
            ...
            if i != self.num_stage-1:
                ...
                # Route reranker (inference only). Ported from the mean-L2 route
                # reranker bundle of 2026-09-23: among the final-stage candidates,
                # rank by model_score - gamma * mean_L2(candidate, route) and
                # replace the model's winner only when an eligible candidate
                # scores strictly higher. Eligibility is an 8 m span of interior
                # projections onto the supplied route for BOTH the winner and the
                # challenger, so with the far-only (42-80 m) route most frames
                # keep the original choice. Everything the selection saw is kept
                # on the head so the driver can log it and the video can draw it.
                ...
                if (getattr(self._config, 'route_rerank_enabled', False)
                    ...
                    select_indices, comparable, costs = select_route_candidate(
                        ...
                        deadband=self._config.route_rerank_deadband,
                        max_distance=self._config.route_rerank_max_distance,
                        adaptive=getattr(self._config, 'route_rerank_adaptive', False),
                        near_distance=getattr(self._config, 'route_rerank_near_distance', 40.),
                        near_gain=getattr(self._config, 'route_rerank_near_gain', 2.),
                        min_travel=getattr(self._config, 'route_rerank_min_travel', 2.))
                    rows = torch.arange(B, device=scores.device)
                    trajs = refinement_dict[-1]['trajs']
                    self._last_route_rerank = {
                        'original_index': original_indices.detach(),
                        'selected_index': select_indices.detach(),
                        'changed': (select_indices != original_indices).detach(),
                        'comparable_original': comparable[rows, original_indices].detach(),
                        'comparable_count': comparable.sum(1).detach(),
                        'candidate_count': torch.full_like(original_indices, scores.shape[1]),
                        'cost_original': costs[rows, original_indices].detach(),
                        'cost_selected': costs[rows, select_indices].detach(),
                        'score_original': scores[rows, original_indices].detach(),
                    ...
                    counts[0] += B
                    counts[1] += int(comparable[rows, original_indices].sum())
                    counts[2] += int((select_indices != original_indices).sum())
                    self._route_rerank_counts = counts
                    print(f'[route-rerank] samples={counts[0]} comparable={counts[1]} changed={counts[2]}', flush=True)
