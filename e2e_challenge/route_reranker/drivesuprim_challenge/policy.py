...
class DriveSuprimPolicy:
    ...
    def __init__(
        ...
        *,
        ...
        # Route reranker (mean L2, inference only). Off unless the container is
        # started with DRIVESUPRIM_ROUTE_RERANK=1, so the same mounted files
        # serve the baseline arm and the reranked arm of a comparison. The
        # weight is the bundle's provisional 0.0005 score penalty per metre.
        config.route_rerank_enabled = (
            os.environ.get("DRIVESUPRIM_ROUTE_RERANK", "0") == "1"
        )
    ...
    def _prediction_to_result(
        ...
        route_rerank = _extract_route_rerank(prediction)
        if route_rerank is not None:
            print(
                "[DriveSuprim] RERANK:"
                f" {'CHANGED' if route_rerank['changed'] else 'kept'}"
                f" comparable={route_rerank['comparable_count']}/{route_rerank['candidate_count']}"
...
def _extract_route_rerank(prediction) -> dict | None:
    """Batch element 0 of the model's route-rerank record, as plain numpy."""
    record = prediction.get("route_rerank")
    if not isinstance(record, dict):
        return None
    out: dict = {}
    for key, value in record.items():
        arr = _tensor_numpy(value)
        if arr is None:
            continue
        arr = np.asarray(arr)
        if arr.ndim >= 1:
            arr = arr[0]
        if key in ("original_traj", "selected_traj"):
            out[key] = np.asarray(arr, dtype=np.float32)[:, :3]
        elif key in ("changed", "comparable_original"):
            out[key] = bool(arr)
        elif key in ("original_index", "selected_index", "comparable_count", "candidate_count"):
            out[key] = int(arr)
        else:
            out[key] = float(arr)
    return out or None
