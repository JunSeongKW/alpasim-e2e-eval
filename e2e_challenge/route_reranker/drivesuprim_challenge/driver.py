...
def _prediction_debug_payload(
    ...
    payload: dict[str, Any] = {
        ...
        # Route reranker record (None when disabled) and the far route message
        # it measured against, so the overlay can draw both winners and the
        # route on one picture.
        "route_rerank": (
            None
            if prediction.route_rerank is None
            else {
                key: (
                ...
                for key, value in prediction.route_rerank.items()
            }
        ),
        "route_message": (
            None
