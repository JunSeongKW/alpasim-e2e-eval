...
class DriveSuprimConfig(TransfuserConfig):
    ...
    # --- route reranker (inference-only, mean L2), from the 2026-09-23 bundle ---
    # Master switch; the challenge driver sets it from DRIVESUPRIM_ROUTE_RERANK.
    route_rerank_enabled: bool = False
    # Legacy adaptive/cost-shaping options are accepted and ignored.
    route_rerank_adaptive: bool = True
    route_rerank_near_distance: float = 40.0
    route_rerank_near_gain: float = 2.0
    route_rerank_min_travel: float = 2.0
    route_rerank_weight: float = 0.0005  # Score penalty per meter of mean L2.
    route_rerank_margin: float = 0.02  # Legacy config compatibility; ignored.
