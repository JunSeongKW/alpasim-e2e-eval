...
from .route_cache import distances_to_polyline
...
class DriveSuprimPrediction:
    ...
    # The corridor gate, applied here rather than in the network: True = the
    # candidate stays inside the cached route's corridor. None when no cached
    # route was available, which is a different thing from "nothing rejected".
    feasibility_route_mask: np.ndarray | None = None
    cached_route: np.ndarray | None = None  # [M,2] rig frame, for the overlay
    # The rejected trajectories themselves, not indices into another array:
    # the refinement candidate set and the exported vocabulary need not be the
    # same size, and an index that silently means the wrong row would draw a
    # confident picture of the wrong thing.
    route_rejected_paths: np.ndarray | None = None  # [R,T,2] rig frame
    # Number of leading candidate poses actually checked by the route gate.
    # The overlay uses this to distinguish the evaluated segment from the
    # untested continuation of the same rejected candidate.
    route_gate_poses: int | None = None
    route_reselected: bool = False
...
class DriveSuprimPolicy:
    ...
    def predict(
        ...
        *,
        ...
        cached_route: np.ndarray | None = None,
        route_corridor_m: float = 4.0,
        route_center_dx_m: float = 0.0,
        lidar2img_by_camera: (
            Mapping[str, np.ndarray]
            | None
        ) = None,
        ...
        # Carried on the instance rather than threaded through every backbone
        # path; the driver holds an inference lock across the whole call, so
        # there is never a second prediction in flight to confuse it with.
        self._cached_route = (
            None if cached_route is None
            else np.asarray(cached_route, dtype=np.float64)[:, :2]
        )
        self._route_corridor_m = float(route_corridor_m)
        self._route_center_dx_m = float(route_center_dx_m)
        ...
        # The remembered route, for the corridor gate inside the model. Kept
        # separate from `route_feature` above: that one is the 42-80 m window
        # the network was trained on and must not change, this one only gates
        # which candidates survive, so the input distribution is untouched.
        cached_route = getattr(self, "_cached_route", None)
        if cached_route is not None and len(cached_route) >= 2:
            features["cached_route"] = torch.as_tensor(
                np.asarray(cached_route, dtype=np.float32),
                dtype=torch.float32,
                device=self._device,
            ).unsqueeze(0)
    ...
    def _prediction_to_result(
        ...
        # The corridor gate itself now lives in the model, folded into
        # feasibility_mask before the coarse->fine top-k. What remains here is
        # only reporting: the overlay still wants to show the route that was
        # used and which vocabulary entries it removed.
        cached_route = getattr(self, "_cached_route", None)
        route_mask = None
        route_rejected = None
        route_gate_poses = None
        route_reselected = False
        corridor = _tensor_numpy(prediction.get("route_corridor_mask"))
        if corridor is not None:
            route_mask = np.asarray(corridor, dtype=np.bool_).reshape(-1)
            if candidate_vocab is not None and route_mask.size == len(candidate_vocab):
                horizon_s = float(os.environ.get(
                    "DRIVESUPRIM_ROUTE_GATE_HORIZON_S", "4.0"
                ))
                dt = float(self._config.trajectory_sampling.interval_length)
                route_gate_poses = min(
                    candidate_vocab.shape[1],
                    max(1, int(round(horizon_s / dt))),
                )
                rejected_vocab = np.asarray(
                    candidate_vocab[~route_mask], dtype=np.float32
                )
                route_rejected = rejected_vocab[:, :, :2].copy()
                # Draw the same AABB-centre points that the gate measured.
                # Candidate vocabulary poses are expressed at the rig origin.
                centre_dx = float(getattr(
                    self._config, "feasibility_ego_center_dx_m", 0.0
                ))
                if abs(centre_dx) > 1e-6 and rejected_vocab.shape[-1] >= 3:
                    heading = rejected_vocab[:, :, 2]
                    route_rejected[:, :, 0] += centre_dx * np.cos(heading)
                    route_rejected[:, :, 1] += centre_dx * np.sin(heading)
                # Cap chosen against what the gate actually rejects. Applied at
                # the vocabulary stage it discards thousands, not the couple of
                # hundred the first version saw, and a cap sized for that
                # reduced the overlay to a handful of lines.
                if len(route_rejected) > 600:
                    route_rejected = route_rejected[
                        np.linspace(0, len(route_rejected) - 1, 600).astype(int)
                    ]
            self._route_log_counter = getattr(self, "_route_log_counter", 0) + 1
            if self._route_log_counter % self._rank_log_every == 0:
                print(
                    f"[DriveSuprim] ROUTEGATE: "
                    f"route_pts={0 if cached_route is None else len(cached_route)}"
                    f" horizon_s={float(os.environ.get('DRIVESUPRIM_ROUTE_GATE_HORIZON_S', '4.0')):g}"
                    f" mode=ego_center"
                    f" kept={int(route_mask.sum())}/{route_mask.size}"
                    f" dropped={int((~route_mask).sum())}",
                    flush=True,
                )
        ...
        return DriveSuprimPrediction(
            ...
            feasibility_route_mask=route_mask,
            cached_route=cached_route,
            route_rejected_paths=route_rejected,
            route_gate_poses=route_gate_poses,
            route_reselected=route_reselected,
    ...
    def _refinement_candidates(self, prediction) -> np.ndarray | None:
        """The candidate set the ranking head scores, as [K,T,>=2]."""
        refinement = prediction.get("refinement")
