...
class DriveSuprimModel(nn.Module):
    ...
    def forward(self,
        ...
        # Route-corridor gate, folded into the same mask as drivable/collision
        # so it lands BEFORE the coarse->fine top-k. Applied after the top-k it
        # would be nearly useless: the 256 that survive were chosen without any
        # knowledge of the route, so on a turn every one of them can be outside
        # the corridor at once. Measured that way, a third of frames had no
        # candidate left to choose from.
        route_ok = self._build_route_corridor_mask(
            features, keyval.shape[0], keyval.device)
        if route_ok is not None:
            output['route_corridor_mask'] = route_ok
            feasibility_mask = (
                route_ok if feasibility_mask is None
                else (feasibility_mask & route_ok)
            )
            output['feasibility_mask'] = feasibility_mask
    ...
    @torch.no_grad()
    def _build_route_corridor_mask(self, features, B, device):
        """Boolean [B, vocab] mask (True = keep) from the driver's cached route.

        The route the runtime hands over starts ~42 m ahead, so the driver
        accumulates it across steps and passes the reconstructed near field in
        as `cached_route` [B, M, 2] in the rig frame, NaN-padded. Absent that
        key -- the feature is off, or the cache has not closed the gap yet --
        this returns None and the candidate set is left alone.

        A candidate is rejected on its WORST sample inside the configured gate
        horizon rather than on its endpoint.  The vocabulary is sampled every
        0.1 s for 4 s, but a long hard gate can discard an otherwise useful
        near-term command because of uncertain motion several seconds away.
        ``DRIVESUPRIM_ROUTE_GATE_HORIZON_S`` limits this check without changing
        the trajectory that is ranked or returned.
        """
        route = features.get('cached_route', None)
        if route is None:
            return None
        route = torch.as_tensor(route, device=device, dtype=torch.float32)
        if route.dim() == 2:
            route = route.unsqueeze(0)
        if route.shape[0] != B or route.shape[-1] < 2:
            return None

        corridor_m = float(os.environ.get('DRIVESUPRIM_ROUTE_CORRIDOR_M', '4.0'))
        horizon_s = float(os.environ.get(
            'DRIVESUPRIM_ROUTE_GATE_HORIZON_S',
            str(self._config.trajectory_sampling.time_horizon),
        ))
        if not np.isfinite(horizon_s) or horizon_s <= 0.0:
            raise ValueError(
                'DRIVESUPRIM_ROUTE_GATE_HORIZON_S must be a positive finite value, '
                f'got {horizon_s!r}'
            )
        dt = float(self._config.trajectory_sampling.interval_length)
        vocab_all = self._trajectory_head.vocab  # [V, P, 3] ego frame (x fwd, y left)
        # Poses represent dt, 2*dt, ..., P*dt, so the first round(horizon/dt)
        # entries are exactly the waypoints at or before the requested horizon.
        gate_poses = min(
            vocab_all.shape[1],
            max(1, int(round(horizon_s / dt))),
        )
        vocab = vocab_all[:, :gate_poses]
        V, P = vocab.shape[0], vocab.shape[1]

        # Match the evaluation metric: it measures the distance from the ego
        # polygon's centroid to the GT route, not the distance from every body
        # corner.  Vocabulary poses are at the rig origin, so translate each
        # pose to the AABB/footprint centre before measuring it.
        cfg = self._config
        dx = float(getattr(cfg, 'feasibility_ego_center_dx_m', 0.0))
        xy = vocab[..., :2].to(device).float()
        th = vocab[..., 2].to(device).float()
        cx, cy = xy[..., 0], xy[..., 1]
        if abs(dx) > 1e-6:
            cx = cx + dx * th.cos()
            cy = cy + dx * th.sin()
        centres = torch.stack([cx, cy], dim=-1)                 # [V, P, 2]
        pts = centres.reshape(1, V * P, 2).to(dtype=torch.float32)

        keep = torch.ones(B, V, dtype=torch.bool, device=device)
        for b in range(B):
            poly = route[b]
            poly = poly[torch.isfinite(poly).all(dim=-1)][:, :2]
            if poly.shape[0] < 2:
                continue
            a = poly[:-1]                     # [S, 2] segment starts
            seg = poly[1:] - a                # [S, 2]
            seg_len_sq = (seg * seg).sum(-1).clamp_min(1e-12)   # [S]

            # Chunked so the [n, S, 2] intermediates stay around 2M elements
            # whatever the route length. Sized in POINTS rather than candidates
            # on purpose: at 40 samples each, a chunk counted in candidates is
            # forty times larger than it reads, which at S=200 is most of a
            # gigabyte per driver -- survivable alone, not with sixteen
            # replicas sharing four cards.
            worst = torch.empty(V, device=device)
            S = max(int(seg.shape[0]), 1)
            group = P                           # samples belonging to one candidate
            pts_per_chunk = max(group, (2_000_000 // S) // group * group)
            for lo in range(0, V * group, pts_per_chunk):
                hi = min(lo + pts_per_chunk, V * group)
                q = pts[0, lo:hi]                                   # [n, 2]
                rel = q[:, None, :] - a[None, :, :]                 # [n, S, 2]
                t = (rel * seg[None, :, :]).sum(-1) / seg_len_sq    # [n, S]
                t = t.clamp(0.0, 1.0)
                closest = a[None, :, :] + t[..., None] * seg[None, :, :]
                dist = (q[:, None, :] - closest).norm(dim=-1)       # [n, S]
                d, idx = dist.min(dim=1)

                # Past the end of what the cache covers, measure only the
                # component perpendicular to the route's final heading. The
                # corridor is then a rectangle open at its short end: running
                # further than the route reaches is not a departure, drifting
                # sideways is -- and at motorway speed the vocabulary reaches
                # 159 m against a route of ~80 m, so without this a candidate
                # is rejected for being fast rather than for being wide.
                #
                # This is also exactly what the scorer does. `_lateral_distance
                # _to_gt` switches to the same perpendicular offset past the end
                # of the recording, so the gate and the metric it is trying to
                # anticipate agree instead of merely resembling each other.
                past_end = (idx == seg.shape[0] - 1) & (
                    t.gather(1, idx[:, None]).squeeze(1) >= 1.0 - 1e-6
                )
                end_dir = seg[-1] / seg[-1].norm().clamp_min(1e-9)
                off = q - poly[-1]
                lateral = (
                    off[:, 0] * end_dir[1] - off[:, 1] * end_dir[0]
                ).abs()
                d = torch.where(past_end, lateral, d)
                # The scorer fails a rollout when the ego centroid reaches or
                # exceeds 4 m, so reject when ANY checked centroid does too.
                worst[lo // group:hi // group] = d.view(-1, group).max(dim=1).values
            keep[b] = worst < corridor_m
        return keep
