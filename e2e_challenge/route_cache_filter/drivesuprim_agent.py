...
class DriveSuprimAgent(AbstractAgent):
    ...
    def forward(self, batch) -> Dict[str, torch.Tensor]:
        ...
        # The driver's remembered route, for the corridor gate in the model.
        # This copy is not optional bookkeeping: the two dicts above are a
        # whitelist, so a key the feature builder adds and this loop does not
        # copy is dropped here silently -- no error, no warning, and a gate
        # downstream that simply never fires. Both dicts get it because
        # inference reads the teacher's prediction, not the student's.
        if 'cached_route' in features:
            for _dict in (teacher_ori_features, student_ori_features):
                _dict['cached_route'] = features['cached_route']
