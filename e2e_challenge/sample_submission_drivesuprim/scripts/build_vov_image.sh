...
IMAGE="${IMAGE:-alpasim-e2e-drivesuprim-vov-driver:latest}"
...
check_asset "${ASSET_DIR}/drivesuprim_vov.ckpt" 1000000000
check_asset "${ASSET_DIR}/dd3d_det_final.pth" 100000000
...
docker build -t "${IMAGE}" \
  -f "${REPO_ROOT}/e2e_challenge/sample_submission_drivesuprim/Dockerfile.vov" \
  "${REPO_ROOT}"
