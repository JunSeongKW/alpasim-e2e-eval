...
from navsim.planning.data.nurec_rig_bbox import rig_bbox_for_log
...
class NavSimScenario(AbstractScenario):
    ...
    def __init__(
        ...
        map_root: str,
        map_version: str,
        ego_vehicle_parameters: Optional[VehicleParameters] = None,
    ) -> None:
        """
        ...
        self._ego_vehicle_parameters = ego_vehicle_parameters or self._vehicle_parameters_from_scene()

    def _vehicle_parameters_from_scene(self) -> VehicleParameters:
        """Use AlpaSim's per-clip rig AABB for NuRec, Pacifica elsewhere.

        The map bundle is the first source.  Bundles extracted before
        ``nurec_map_data.py`` started writing ``rig_bbox`` carry no such key, so
        a side-car table built from the same USDZ stands in -- otherwise every
        NuRec clip silently falls back to a Pacifica that is 8-15% wider than
        the rig that recorded it.  See ``nurec_rig_bbox`` and
        ``docs/nurec/NC_HUMAN_FILTER.md`` section 6.
        """
        rig_bbox = getattr(self._scene.map_api, "rig_bbox", None)
        if not rig_bbox or len(rig_bbox.get("dim") or ()) < 3:
            rig_bbox = rig_bbox_for_log(self._log_name)
        if not rig_bbox or len(rig_bbox.get("dim") or ()) < 3:
            return get_pacifica_parameters()
        rotation = rig_bbox.get("rot") or (0.0, 0.0, 0.0)
        if any(abs(float(value)) >= 1e-4 for value in rotation):
            warnings.warn("rotated NuRec rig_bbox is unsupported; using Pacifica parameters")
            return get_pacifica_parameters()
        dimensions = [float(value) for value in rig_bbox["dim"]]
        centroid = [float(value) for value in (rig_bbox.get("centroid") or (0.0, 0.0, 0.0))]
        # Match AlpaSim EvalConfig: 2% shrink, centered on rig_bbox.centroid.
        length, width = dimensions[0] * 0.98, dimensions[1] * 0.98
        center_x = centroid[0]
        pacifica = get_pacifica_parameters()
        return VehicleParameters(
            width=width,
            front_length=center_x + length / 2.0,
            rear_length=length / 2.0 - center_x,
            cog_position_from_rear_axle=pacifica.cog_position_from_rear_axle,
            wheel_base=pacifica.wheel_base,
            height=dimensions[2],
            vehicle_name="nurec-rig-bbox",
            vehicle_type="alpasim",
        )
    ...
    def map_api(self) -> AbstractMap:
        """Inherited, see superclass."""
        # Scene construction already resolved clip-local NuRec maps.  Returning
        # that instance also preserves it when metric caching runs in workers.
        if self._map_name.startswith("nurec:"):
