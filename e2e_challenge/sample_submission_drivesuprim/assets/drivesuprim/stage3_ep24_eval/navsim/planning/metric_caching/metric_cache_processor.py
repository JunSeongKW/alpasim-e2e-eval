...
MAX_EMPTY_TAIL_HOLD_S = 1.0  # [s]
"""How far past the last real observation the empty-tail fill is allowed to reach.

Every NuRec log carries no annotations at all in its first and last frame, and the
last frame is a *sample* of exactly one window per clip -- the one anchored 4 s
before the log ends.  Interpolation only spans a track's own observations, so for
that window every box vanishes from t = 3.6 s onward and the last 5 of 41 steps
are scored against an empty scene.  Measured on 140 such windows, filling the gap
turns 7.1% of the reachable ``no_at_fault_collisions`` = 1 candidates into
collisions, and 88% of the windows contain at least one -- always in the same
direction, because a missing box can only ever forgive.

Filling it is a half-second step off a track that the window has already seen more
than once, which the annotated speed is good to about 0.05-0.11 m/s over.  That is
a different claim from the four-second hold this file removes above, where a
single unrepeatable detection was asserted across the whole horizon.  Beyond this
limit the run of empty frames is no longer a boundary artefact and nothing here
guesses at it.
"""


def _last_observed_time(gt_detection_tracks, relative_time_s) -> Optional[float]:
    """Time of the last sample that carries any annotation, if the tail is empty.

    Returns ``None`` when the final sample has objects -- the ordinary case, where
    a track ending early really did leave and must not be extended.
    """
    last = None
    for index in range(len(gt_detection_tracks) - 1, -1, -1):
        if len(gt_detection_tracks[index].tracked_objects) > 0:
            last = index
            break
    if last is None or last == len(gt_detection_tracks) - 1:
        return None
    return float(relative_time_s[last])


def _extend_over_empty_tail(detection_tracks_states, last_observed_time, time_horizon) -> int:
    """Carries each still-present track across the empty tail at its own speed.

    Only tracks seen more than once are extended: a single-sample box is decided by
    the support-window rule above, and giving it a second state here would smuggle
    it past that rule.  A track that had already ended before the tail is left
    ended.  Static objects carry zero velocity and so simply hold their place.
    """
    horizon_gap = time_horizon - last_observed_time
    if horizon_gap <= 0.0 or horizon_gap > MAX_EMPTY_TAIL_HOLD_S:
        return 0
    extended = 0
    for states in detection_tracks_states.values():
        if len(states) < 2 or abs(states[-1][0] - last_observed_time) > 1e-6:
            continue
        tail_state = states[-1].copy()
        tail_state[0] = time_horizon
        tail_state[1] += states[-1][4] * horizon_gap
        tail_state[2] += states[-1][5] * horizon_gap
...
class MetricCacheProcessor:
    ...
    def _interpolate_gt_observation(self, scenario: NavSimScenario) -> List[DetectionsTracks]:
        ...
        # A wholly empty sample says the annotation is missing, not that the road
        # cleared; without this the window's last 0.5 s is scored against nothing.
        last_observed_time = _last_observed_time(gt_detection_tracks, relative_time_s)
        if last_observed_time is not None:
            _extend_over_empty_tail(detection_tracks_states, last_observed_time, time_horizon)
    ...
    def _road_edges_for(self, scenario: NavSimScenario) -> Dict[str, Any]:
        ...
        # No height filter here.  Which boundaries belong to the ego's road is a
        # question about a body at a moment, not about the frame: on the steeper
        # clips the recorded run climbs a median 4.74 m within its own 4 s and up
        # to 12.69 m, so any single reference height taken at t=0 is wrong for
        # most of the horizon and a +-3 m gate around it discards the road the
        # ego is driving onto.  The lines and their vertex heights are stored
        # whole; the scorer matches each against the road surface beneath the
        # body it is testing (see PDMScorer._road_edge_compliance).
        lines, elevations, profiles = [], [], []
        for boundary in boundaries:
            ...
            profiles.append(getattr(polyline, "height_profile", None))
        if not lines:
            return {}

        # The vertical frame the scorer resolves "the road under this body"
        # against.  Lane centrelines are the only surface the bundle gives a
        # height for, so they travel with the edges rather than being re-queried
        # at scoring time.
        surfaces, surface_profiles = [], []
        for layer in (SemanticMapLayer.LANE, SemanticMapLayer.LANE_CONNECTOR):
            for lane in map_api.get_proximal_map_objects(
                scenario.initial_ego_state.center.point, self._map_radius, [layer]
            ).get(layer, []):
                path = getattr(lane, "baseline_path", None)
                profile = getattr(path, "height_profile", None) if path else None
                if path is None or profile is None or path.linestring.is_empty:
                    continue
                surfaces.append(path.linestring)
                surface_profiles.append(profile)

        return {
            "road_edges": lines,
            "road_edge_elevations": np.asarray(elevations, dtype=np.float64),
            "road_edge_profiles": profiles,
            "road_surfaces": surfaces,
            "road_surface_profiles": surface_profiles,
        }
