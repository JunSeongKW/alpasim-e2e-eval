...
class MetricCacheProcessorAugTrain:
    ...
    def _interpolate_gt_observation(self, scenario: NavSimScenario) -> List[DetectionsTracks]:
        ...
        # 통째로 빈 샘플은 "도로가 비었다"가 아니라 "라벨이 없다"는 뜻이다.  이걸 안
        # 채우면 그 창의 마지막 0.5 s 가 아무것도 없는 장면으로 채점된다.  규칙과
        # 근거는 metric_cache_processor 쪽에 있고, 여기서는 그것을 그대로 쓴다.
        ...
        for time_s in interpolated_time_s:
            ...
            for token, interpolator in detection_interpolators.items():
                ...
                if interpolator.start_time == interpolator.end_time:
                    ...
                    if is_track_stopped(initial_detection_track, SINGLE_SAMPLE_STOPPED_SPEED) or (
                        abs(time_s - interpolator.start_time) <= resolution_step / 2.0
                    ):
