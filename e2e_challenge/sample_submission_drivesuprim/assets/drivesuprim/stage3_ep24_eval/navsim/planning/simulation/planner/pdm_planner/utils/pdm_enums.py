...
class MultiMetricIndex(IntEnum):
    ...
    DRIVING_DIRECTION = 3
    # AlpaSim ends a scene at 4 m of lateral drift from the recorded run, on the
    # same footing as a collision or leaving the road.  Multiplicative, so a
    # proposal that wanders that far scores zero whatever else it did.
    GT_COMPLIANCE = 4
