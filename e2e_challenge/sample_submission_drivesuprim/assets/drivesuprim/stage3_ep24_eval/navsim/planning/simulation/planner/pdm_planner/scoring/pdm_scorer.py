...
GT_PROGRESS_LATERAL_CUTOFF = os.environ.get("NUREC_EP_LATERAL_CUTOFF", "1") == "1"
"""Stop crediting progress once a proposal leaves the recorded run's corridor.

AlpaSim's aggregation applies ``RemoveTimestepsAfterEvent(dist_to_gt_trajectory
>= max_dist_to_gt_trajectory)`` before taking progress as LAST, so leaving the
corridor freezes progress where it happened.  Our labels only ever cut at
contact, and projecting the rest of the horizon regardless is what let a
proposal drive 147 m the wrong way and still bank the distance.

Measured over 300 frames, adding the cutoff takes progress from 81 distinct
values per frame to 495, drops the share of candidates pinned at 1.0 from 70.7%
to 3.4%, and scores a candidate that leaves the map at 0.034 instead of 0.737.
It also ends the human penalty filter firing on 45.3% of frames -- with the
corridor honoured the recorded run no longer fails its own progress term, so
those frames stop having all four terms overwritten with ones.

Set NUREC_EP_LATERAL_CUTOFF=0 to score the way the earlier labels were built.
"""
...
LEVEL_RULE_T0 = os.environ.get("NUREC_LEVEL_RULE", "body") == "t0"
"""Resolve every body against the ego's height at t=0 instead of its own.

Diagnostic only.  The recorded run climbs a median 4.74 m inside its own 4 s on
the steepest clips, so a t=0 reference is wrong for most of the horizon; this
exists to measure what that costs.
"""

ROAD_EDGE_ELEVATION_GATE_M = 3.0  # [m]
"""Height difference above which a boundary belongs to a different road level.

Compared against the road directly under the body being tested, never against a
frame-wide reference: this separates decks, it is not a budget for grade.
"""

NEAR_SURFACE_FOR_LEVEL_M = 25.0  # [m]
"""How close a lane must be for it to describe the road height under a body.

Beyond this there is no surface to resolve against, and the level test abstains
rather than guessing -- a body out in a car park is judged by the flat rule.
"""
...
class PDMScorer:
    ...
    def __init__(
        ...
        self._edge_profiles: List[Any] = []
        self._surfaces: List[Any] = []
        self._surface_profiles: List[Any] = []
        self._road_surface_tree: Optional[STRtree] = None
        self._frame_surface_z: Any = _UNSET
        self._level_cache: dict = {}
        self._ego_level: Optional[float] = None
    ...
    def set_road_levels(self, edge_profiles, surfaces, surface_profiles) -> None:
        """Height data for deciding which boundaries belong to a body's own road.

        Without it every boundary within the map radius is tested against every
        body, so a flyover's parapet marks the road beneath it as off-road.  The
        obvious fix -- one reference height per frame -- does not survive grade:
        the recorded run climbs a median 4.74 m inside its own 4 s on the
        steepest clips and up to 12.69 m, so a height taken at t=0 is wrong for
        most of the horizon.  Instead each body is resolved against the road
        directly beneath it, which is local and therefore holds however the clip
        climbs.

        Passing anything empty leaves the plain flat test in place.
        """
        if not surfaces or not edge_profiles:
            self._road_surface_tree = None
            self._frame_surface_z = _UNSET
            return
        self._frame_surface_z = _UNSET
        self._edge_profiles = edge_profiles
        self._surfaces = surfaces
        self._surface_profiles = surface_profiles
        self._road_surface_tree = STRtree(surfaces)

    def _height_at(self, profile, station: float) -> Optional[float]:
        """Height of a polyline at an arc length, or None when it carries none."""
        if profile is None:
            return None
        stations, heights = profile
        return float(np.interp(station, stations, heights))

    def _surface_height_under(self, point: Point,
                              expected_z: Optional[float] = None) -> Optional[float]:
        """Height of the road surface beneath a point, or None if there is none.

        "Beneath" cannot be settled in plan view alone.  Under an overpass the
        deck's own lanes sit directly above the road and are the nearest thing
        to the body in 2D, so taking the nearest surface returns the bridge --
        and then the bridge's parapet matches its own deck and is tested against
        a car underneath it.  Measured on one such frame, 40 of the 63 boundaries
        in view belonged to the deck and all but one passed that way.

        ``expected_z`` breaks the tie: among the surfaces near the point, prefer
        the one closest to the height the body is actually at.  Ties are only
        broken vertically -- the nearest surface still wins whenever the levels
        are indistinguishable.
        """
        tree = getattr(self, "_road_surface_tree", None)
        if tree is None:
            return None
        found = tree.query(point.buffer(NEAR_SURFACE_FOR_LEVEL_M))
        if not len(found):
            return None
        best_idx, best_key = None, None
        for idx in found:
            gap = self._surfaces[idx].distance(point)
            if gap > NEAR_SURFACE_FOR_LEVEL_M:
                continue
            height = self._height_at(
                self._surface_profiles[idx], self._surfaces[idx].project(point)
            )
            if expected_z is None or height is None:
                key = (0.0, gap)
            else:
                # Level first, then proximity within that level.
                key = (abs(height - expected_z) > ROAD_EDGE_ELEVATION_GATE_M, gap)
            if best_key is None or key < best_key:
                best_idx, best_key = idx, key
        if best_idx is None:
            return None
        surface = self._surfaces[best_idx]
        return self._height_at(
            self._surface_profiles[best_idx], surface.project(point)
        )

    def _body_level(self, proposal_idx: int, flat_idx: int,
                    centre: Point) -> Optional[float]:
        """The height this body is expected to be at, walked along its own path.

        The level cannot be pinned at the ego and it cannot be read off the
        nearest surface either.  Pinning fails on grade -- the road climbs away
        from the reference and its own boundaries start reading as another deck,
        which scored the recorded human off-road on 5 of its 80 bodies on the
        steepest clip.  Taking the nearest surface fails under a bridge, where
        the deck is nearer in plan view than the road the body is on.

        Walking the path settles both: each pose resolves against the height the
        previous pose reached, so a climb is followed while an 8 m jump onto a
        deck is refused.  Only poses up to this one are walked, and the result is
        cached, so each body costs one surface lookup.
        """
        cache = self._level_cache
        hit = cache.get(flat_idx)
        if hit is not None:
            return hit[0]
        stride = self._ego_polygons.shape[1]
        first = proposal_idx * stride
        # Walk forward from the last pose of this proposal already resolved.
        start, level = first, self._ego_level
        for step in range(flat_idx - 1, first - 1, -1):
            previous = cache.get(step)
            if previous is not None:
                start, level = step + 1, previous[0]
                break
        centroids = self._ego_polygons
        for step in range(start, flat_idx):
            point = centroids[step // stride, step % stride].centroid
            resolved = self._surface_height_under(point, level)
            if resolved is not None:
                level = resolved
            cache[step] = (level,)
        resolved = self._surface_height_under(centre, level)
        if resolved is not None:
            level = resolved
        cache[flat_idx] = (level,)
        return level

    def _edge_is_on_level(self, edge_idx: int, body_point: Point,
                          surface_z: Optional[float]) -> bool:
        """Whether a boundary runs at the height of the road under this body."""
        if surface_z is None:
            return True
        edge = self._road_edges[edge_idx]
        edge_z = self._height_at(
            self._edge_profiles[edge_idx] if edge_idx < len(self._edge_profiles) else None,
            edge.project(body_point),
        )
        if edge_z is None:
            return True
        return abs(edge_z - surface_z) <= ROAD_EDGE_ELEVATION_GATE_M
    ...
    def score_proposals(
        ...
        # gt_compliance is gone: AlpaSim has no drift term, the 4 m corridor
        # enters through progress instead, and as a separate factor it did
        # nothing but fire the human penalty filter -- measured, it was the
        # trigger on 297 of 300 frames, because the recorded run cannot hold
        # its own corridor either.  The index stays in the enum so the arrays
        # keep their shape; pinning it at one keeps it out of every product.
        self._multi_metrics[MultiMetricIndex.GT_COMPLIANCE] = np.ones(
    ...
    def _aggregate_pdm_scores(self) -> npt.NDArray[np.float64]:
        ...
        if self._gt_path is not None:
            ...
            # GT compliance is not a factor here.  AlpaSim has no such term --
            # its scene score is the hard failures times progress -- and the 4 m
            # corridor enters instead by freezing progress at the moment a body
            # leaves it, which is what RemoveTimestepsAfterEvent does upstream of
            # its aggregation.  Kept as its own factor it did nothing but fire
            # the human penalty filter: measured over 300 frames it was constant
            # at 1.0 on 297 of them, because the recorded run cannot hold its own
            # 4 m corridor either.  It is still computed and exported so the
            # drift can be read, but the score no longer multiplies by it.
            return (
    ...
    def _road_edge_compliance(self) -> npt.NDArray[np.float64]:
        ...
        """
        # The ego's own level seeds every proposal: they all start from it.
        self._level_cache = {}
        self._ego_level = self._surface_height_under(
            Point(self._states[0, 0, StateIndex.X], self._states[0, 0, StateIndex.Y])
        )
        ...
        # A body only reaches this loop by touching an edge, so anywhere the map
        # draws no boundary is unscored: measured on one clip, 19.3% of
        # candidates ended more than 5 m clear of every lane surface and still
        # kept DAC=1, and 78.6% of those had no edge within 10 m to touch.  The
        # edges are sparse because the NuRec maps carry almost no DRIVABLE_AREA
        # layer (2 polygons against 68 lanes on the clip checked).  Where the
        # boundary is missing, fall back to the lanes: a body with no lane
        # surface anywhere near it is off the road whether or not a line says so.
        for flat_idx in np.unique(candidates):
            ...
            hits = self._road_edge_tree.query(body)
            if not len(hits):
                continue
            # Resolved once per body, not once per edge: the surface underneath
            # does not depend on which boundary is being considered.
            centre = body.centroid
            # Which level the body is on has to be carried forward from where it
            # started, because the trajectory itself is planar: the vocabulary
            # is (x, y, heading) and says nothing about height.  Following the
            # road from the ego's own level gives each body an expected height,
            # which is what tells a car under a bridge from one on it.
            expected = self._body_level(proposal_idx, flat_idx, centre)
            # NUREC_LEVEL_RULE=t0 pins every body to the height under the ego at
            # t=0, which is what the frame-wide gate used to do -- kept as a
            # switch so the two can be scored against each other on one cache.
            if LEVEL_RULE_T0:
                if self._frame_surface_z is _UNSET:
                    origin = Point(
                        self._states[0, 0, StateIndex.X], self._states[0, 0, StateIndex.Y]
                    )
                    self._frame_surface_z = self._surface_height_under(origin)
                surface_z = self._frame_surface_z
            else:
                surface_z = self._surface_height_under(centre, expected)
            for edge_idx in hits:
                if self._road_edges[edge_idx].distance(body) >= ROAD_EDGE_CONTACT_M:
                    continue
                if not self._edge_is_on_level(edge_idx, centre, surface_z):
                    continue
                scores[proposal_idx] = 0.0
                break
        return scores
    ...
    def _gt_progress(self) -> npt.NDArray[np.float64]:
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            if GT_PROGRESS_LATERAL_CUTOFF:
                # AlpaSim deletes every timestep after the ego first reaches
                # GT_LATERAL_LIMIT_M from the recorded run, then takes the last
                # surviving progress.  Projecting the whole horizon regardless
                # credits a proposal that left the corridor with progress the
                # simulator would have stopped counting -- measured on the label
                # set, a proposal ending 12 m the wrong way still scored 0.748.
                for t in range(time_idx + 1):
                    if self._gt_path.distance(Point(*centres[proposal_idx, t])) >= GT_LATERAL_LIMIT_M:
                        time_idx = max(0, t - 1)
                        break
            ...
            if os.environ.get("NUREC_EP_TRACE2") == "1" and proposal_idx in (366, 1262, 461, 3704):
                _c0 = centres[proposal_idx, 0]; _ce = centres[proposal_idx, time_idx]
                _cl = centres[proposal_idx, -1]
                print(f"[EP2] prop={proposal_idx} n={self._num_proposals} "
                      f"len={self._gt_path.length:.1f} start={start:.2f} rem={remaining:.2f} "
                      f"contact={self._contact_time_idcs[proposal_idx]} tidx={time_idx} "
                      f"made={made_good:.2f} ep={scores[proposal_idx]:.3f} "
                      f"c0=({_c0[0]:.1f},{_c0[1]:.1f}) ct=({_ce[0]:.1f},{_ce[1]:.1f}) "
                      f"cend=({_cl[0]:.1f},{_cl[1]:.1f})", flush=True)
        return scores
