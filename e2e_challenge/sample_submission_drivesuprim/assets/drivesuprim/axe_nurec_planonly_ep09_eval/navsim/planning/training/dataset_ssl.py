...
class DatasetSSL(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        """Robust wrapper: transient data/map errors (e.g. NFS+SQLite GPKG
        "database disk image is malformed" under concurrent access) must not kill
        a multi-hour training run. On failure, retry a different random sample a
        few times before finally re-raising (so genuine per-sample bugs still
        surface)."""
        import random
        from navsim.agents.drivesuprim.drivesuprim_features import CalibrationMismatch
        n = len(self._scene_loader.tokens)
        cur = idx
        for attempt in range(8):
            try:
                return self._getitem_impl(cur)
            except CalibrationMismatch:
                # Systematic, not transient: every sample in the split carries
                # the same calibration, so retrying would only bury the message
                # under eight identical failures and then raise anyway.
                raise
            except Exception as e:
                print(f"[dataset] __getitem__ idx={cur} failed "
                      f"({type(e).__name__}: {str(e)[:100]}); retry {attempt+1}/8 with a different sample",
                      flush=True)
                cur = random.randint(0, n - 1)
        # exhausted retries -> re-raise on the original index
        return self._getitem_impl(idx)

    def _getitem_impl(self, idx: int) -> Tuple[Dict[str, torch.Tensor], Dict[str, torch.Tensor], str]:
        """
