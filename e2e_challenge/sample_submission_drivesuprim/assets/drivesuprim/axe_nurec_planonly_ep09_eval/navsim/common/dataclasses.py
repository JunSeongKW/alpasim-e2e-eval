...
def _empty_route() -> npt.NDArray[np.float32]:
    """An all-NaN route, i.e. "no route known at this frame"."""
    return np.full((ROUTE_SLOTS, 3), np.nan, dtype=np.float32)


def route_waypoints_of(scene_frame: Dict[str, Any]) -> npt.NDArray[np.float32]:
    """Route message stored on a scene frame, or an empty one.

    Logs prepared before routes existed simply have no such key; the all-NaN
    fallback keeps them loadable and the consumer's mask already handles a route
    with no valid waypoint.
    """
    waypoints = scene_frame.get("route_waypoints")
...
class EgoStatus:
    ...
    # PAI-Track route for this timestamp, in the ego rig frame: x forward,
    # y left, z zero, with unused slots NaN.
    route_waypoints: npt.NDArray[np.float32] = field(default_factory=_empty_route)
    ...
    def from_scene_dict_list(
        ...
        for frame_idx in range(num_history_frames):
            ...
            ego_status = EgoStatus(
                ...
                route_waypoints=route_waypoints_of(scene_dict_list[frame_idx]),
            )
...
class Scene:
        ...
        for frame_idx in range(self.scene_metadata.num_history_frames):
            ...
            ego_statuses.append(
                EgoStatus(
                    ...
                    route_waypoints=frame_ego_status.route_waypoints,
                )
            )
    ...
    def _build_map_api(cls, map_name: str) -> AbstractMap:
        ...
        # NuRec reconstructions carry a clip-local vector map rather than one
        # of nuPlan's city-wide GPKG maps.  Keep the regular NAVSIM path intact
        # and dispatch only explicit ``nurec:<clip-id>`` names to the adapter.
        from navsim.planning.data.nurec_map import is_nurec_map_name, load_nurec_map

        if is_nurec_map_name(map_name):
            return load_nurec_map(map_name)
    ...
    def _build_ego_status(cls, scene_frame: Dict) -> EgoStatus:
        ...
        return EgoStatus(
            ...
            route_waypoints=route_waypoints_of(scene_frame),
        )
...
class PDMResults:
    ...
    # Raw terms for the v1-style PDM score, in order:
    ...
    # Kept as an array so human_penalty_filter leaves it untouched (it rewrites
    # scalar columns to 1.0), letting pdms_v1 be aggregated from raw values the
    # same way `score` is.
    v1_terms: npt.NDArray[np.float64]
    ...
    def get_empty_results(cls) -> PDMResults:
        ...
        return PDMResults(
            ...
            v1_terms=np.full(6, np.nan, dtype=np.float64),
            pdm_score=np.nan,
        )
