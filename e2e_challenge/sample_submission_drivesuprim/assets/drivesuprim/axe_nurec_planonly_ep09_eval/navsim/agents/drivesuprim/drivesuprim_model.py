...
def _rank_product(config, result, use_traffic_light: bool):
...
def _rank_imi_term(config, result):
    ...
    """
    if not config_imi_rank_weight(config):
        # Weight zero means the term is off. Returning zeros rather than
        # multiplying keeps the log path from evaluating 0 * log(0): a softmax
        # entry that underflows gives -inf, and 0 * -inf is NaN, which would
        # poison the whole ranking instead of removing one term from it.
        return torch.zeros_like(result['imi'])
    if getattr(config, 'pdm_rank_product', False):
        ...
        return config_imi_rank_weight(config) * weights
    return config_imi_rank_weight(config) * result['imi'].softmax(-1).log()


def _rank_score(config, result, use_traffic_light: bool):
    ...
    """
    if getattr(config, 'pdm_rank_product', False):
        return _rank_product(config, result, use_traffic_light)
    score = None
...
class DriveSuprimModel(nn.Module):
    def __init__(self, config: DriveSuprimConfig):
        ...
        assert config.backbone_type in ['vit', 'vov', 'resnet34', 'resnet50', 'bevformer_m']
    ...
    def _build_feasibility_mask(self, output, B, device):
        ...
        key = (s, mrg, cfg.feasibility_ego_length, cfg.feasibility_ego_width,
        ...
        if getattr(self, '_ego_key', None) != key or getattr(self, '_ego_corners', None) is None \
            ...
            ego_c = self._obb_corners(xy[..., 0], xy[..., 1], th, ehl, ehw)   # [V,P,4,2]
        ...
        if cfg.feasibility_drivable and 'bev_seg_map' in output:
            ...
            feasible &= ~off.any(dim=3).any(dim=2)                            # any corner, any pose
    ...
    def forward(self, img_feat, refinement_dict, route=None, route_mask=None) -> Dict[str, torch.Tensor]:
        ...
        for i in range(self.num_stage):
            ...
            if self._config.refinement.use_imi_learning_in_refinement:
                scores += _rank_imi_term(self._config, last_layer_result)
