...
def aggregate_epdms_labels(gt: Dict[str, np.ndarray], scorer_config) -> np.ndarray:
    ...
    GT compliance multiplies: drifting 4 m off the recorded run ends the scene in
    AlpaSim the same way a collision does.
    """
    terms = [gt["no_at_fault_collisions"], gt["drivable_area_compliance"]]
    # Labels written before drift became a term carry no gt_compliance.  Absent
    # means the term never fired, which is a factor of one.
    drift = gt.get("gt_compliance")
    terms.append(np.ones_like(terms[0]) if drift is None else drift)
...
ALPASIM_SCORE_TERMS = ("no_at_fault_collisions", "drivable_area_compliance", "gt_compliance", "ego_progress")
"""Columns whose product is the scene score.  Same four terms, same order, as
...
def pdm_score_full_v2(
    ...
    gt = {
        ...
        'gt_compliance': scorer._multi_metrics[MultiMetricIndex.GT_COMPLIANCE].astype(np.float16)[1:],
