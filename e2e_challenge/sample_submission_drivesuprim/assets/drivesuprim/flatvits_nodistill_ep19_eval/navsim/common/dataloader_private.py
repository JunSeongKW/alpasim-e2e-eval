...
class MetricCacheLoader:
    ...
    def _load_metric_cache_paths(self, cache_path: Path) -> Dict[str, Path]:
        ...
        # Metadata CSVs may come from another machine and contain stale absolute
        # roots. The cache layout is stable under
        # cfg.metric_cache_path, so resolve every row relative to the supplied
        # cache_path using the final log/unknown/token/metric_cache.pkl suffix.
        root = Path(cache_path)
        metric_cache_dict = {}
        for stored_path in cache_paths:
            parts = Path(stored_path).parts
            if len(parts) >= 4:
                local_path = root.joinpath(*parts[-4:])
            else:
                local_path = Path(stored_path)
            metric_cache_dict[parts[-2]] = str(local_path)
