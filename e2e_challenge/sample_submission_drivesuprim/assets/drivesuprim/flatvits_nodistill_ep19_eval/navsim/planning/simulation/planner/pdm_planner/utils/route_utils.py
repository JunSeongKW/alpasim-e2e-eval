...
def _is_roadblock_connector(roadblock: RoadBlockGraphEdgeMapObject) -> bool:
    """Whether a roadblock is a connector, across map implementations."""
    is_connector = getattr(roadblock, "is_connector", None)
    if is_connector is not None:
        return bool(is_connector)
    return "Connector" in type(roadblock).__name__
...
def remove_route_loops(
    ...
    for idx, roadblock in enumerate(route_roadblocks):
        ...
        # Asking the object whether it is a connector, rather than matching one
        # map's class name: the name test silently held for every NuRec route,
        # whose connectors are NuRecRoadBlock, so no NuRec loop was ever cut.
        if _is_roadblock_connector(roadblock):
