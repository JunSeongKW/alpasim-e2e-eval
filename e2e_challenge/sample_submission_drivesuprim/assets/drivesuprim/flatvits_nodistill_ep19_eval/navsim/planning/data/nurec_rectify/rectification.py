...
# Copied verbatim from the AlpaSim challenge driver's
# ``drivesuprim_challenge/rectification.py`` so the images this repository trains
# on are rectified by the same code that rectifies them at evaluation time. The
# only edit is the import above, which points at a local stand-in for the
# protobuf camera model instead of the gRPC package.
# ``tests/test_nurec_rectification.py`` diffs this file against the original and
# compares their output pixel for pixel -- keep the two in step.
