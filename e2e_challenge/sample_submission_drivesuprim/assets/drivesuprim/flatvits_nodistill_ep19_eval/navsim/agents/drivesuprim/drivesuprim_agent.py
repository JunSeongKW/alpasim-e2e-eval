    ...
    def compute_loss(
        ...
        if self._config.use_aux_heads and 'bev_seg_map' in ori_predictions:
            ...
            if getattr(self._config, 'aux_log_metrics', True):
                ...
                self._aux_metric_step = getattr(self, '_aux_metric_step', 0) + 1
                every = max(1, int(getattr(self._config, 'aux_metric_every_n_steps', 1)))
                if self._aux_metric_step % every == 0:
