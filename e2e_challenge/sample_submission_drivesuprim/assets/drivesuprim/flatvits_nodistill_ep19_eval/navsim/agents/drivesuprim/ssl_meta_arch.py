...
class SSLMetaArch(nn.Module):
    ...
    def forward(self, teacher_ori_features, student_feat_dict_lst, **kwargs):
        ...
        if self.cfg.training:
            # The teacher's output feeds the self-distillation targets ONLY, and
            # those are planning quantities (trajectory + PDM scores). With
            # planning_loss_weight = 0 (stage 1 of the three-stage schedule) the
            # resulting loss is multiplied by zero, so this whole extra BEV
            # extraction -- image backbone + encoder over every camera and frame
            # -- would be computed and thrown away.
            if float(getattr(self.cfg, "planning_loss_weight", 1.0)) == 0.0 \
                    or not bool(getattr(self.cfg, "use_soft_teacher", True)):
                teacher_pred = None
            else:
